<header class="bg-white rounded-2xl shadow-sm p-4"><h2 class="text-base font-bold">📜 History</h2></header>
...
<form method="GET" class="bg-white rounded-2xl shadow-sm p-4 space-y-2 text-sm">
  ...
  <select name="class_id" class="w-full border rounded-xl p-2">
    ...
    <?php foreach ($classes as $c): ?>
      <option value="<?= $c['id'] ?>" <?= ((int)($filters['class_id'] ?? 0)===(int)$c['id'])?'selected':'' ?>><?= h($c['class_name']) ?></option>
    <?php endforeach; ?>
  </select>
  ...
  <select name="student_id" class="w-full border rounded-xl p-2">
    ...
    <?php foreach ($students as $s): ?>
      <option value="<?= $s['id'] ?>" <?= ((int)($filters['student_id'] ?? 0)===(int)$s['id'])?'selected':'' ?>><?= h($s['name']) ?></option>
    <?php endforeach; ?>
  </select>
  <div class="flex gap-2">
    <button class="flex-1 rounded-xl bg-emerald-600 text-white p-2">Apply</button>
    <a href="<?= app_url('index.php?page=history&export=csv') ?>" class="flex-1 rounded-xl bg-amber-500 text-white p-2 text-center">Export</a>
  </div>
</form>
...
<article class="bg-white rounded-2xl shadow-sm p-4 space-y-2 text-sm">
  ...
  <div class="flex gap-2 overflow-x-auto text-xs">
    <?php foreach (['subah'=>'Subah','dhuhr'=>'Dhuhr','asr'=>'Asr','maghrib'=>'Maghrib','isha'=>'Isha'] as $k=>$label): ?>
      <span class="px-2 py-1 rounded-full <?= ((int)$r[$k]===1)?'bg-emerald-100 text-emerald-700':'bg-slate-200 text-slate-600' ?>"><?= $label ?></span>
    <?php endforeach; ?>
  </div>
</article>
<?php endforeach; ?>
