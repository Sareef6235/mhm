...
<form method="GET" class="grid grid-cols-2 gap-2">
  ...
  <?php foreach ($classes as $c): ?>
    <button name="class_id" value="<?= $c['id'] ?>" class="rounded-2xl bg-white p-3 text-sm shadow-sm transition hover:-translate-y-0.5 <?= ((int)$selectedClass===(int)$c['id'])?'ring-2 ring-emerald-500':'' ?>">
      <?= h($c['class_name']) ?>
    </button>
  <?php endforeach; ?>
</form>
...
<form method="GET" class="bg-white rounded-2xl shadow-sm p-3">
  ...
  <input name="search" value="<?= h($search) ?>" class="w-full border rounded-xl p-2 text-sm" placeholder="Search students">
</form>

<section class="space-y-2">
  <?php foreach ($students as $s): ?>
    <a href="<?= app_url('index.php?page=record&student_id=' . (int)$s['id']) ?>" class="block bg-white rounded-2xl shadow-sm p-3 transition hover:-translate-y-0.5">
