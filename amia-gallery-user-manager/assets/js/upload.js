(function($){'use strict';
  ...
  $(document).on('submit','.agum-user-form',function(e){
    ...
    ['student_id','admission_no','name','class','dob','role','username','email','phone_number'].forEach(function(name){ if(!form.find('[name="'+name+'"]').val()){ missing.push(name); } });
...
(function($){'use strict';
  $(document).on('change','input[name="bulk_images[]"]',function(){
    const preview=$(this).closest('form').find('.agum-bulk-preview').empty();
    Array.from(this.files||[]).slice(0,24).forEach(function(file){
      if(file.type.indexOf('image/')===0){ preview.append($('<img alt="Bulk preview">').attr('src',URL.createObjectURL(file))); }
    });
  });
})(jQuery);
